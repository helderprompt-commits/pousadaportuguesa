    ...
    <section className="py-24 bg-white border-t border-zinc-100">
      <div className="max-w-7xl mx-auto px-6 md:px-16 lg:px-20">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-14 gap-4">
          <div>
            <span className="text-secondary text-xs font-bold uppercase tracking-[0.2em] mb-3 block">Insights & Notícias</span>
            <h2 className="text-3xl md:text-4xl font-extrabold text-zinc-900 tracking-tight">
              Blog do Residencial Luso
            </h2>
          </div>
          <Link href="/blog" className="text-primary font-semibold text-sm flex items-center gap-2 hover:gap-3 transition-all group">
          ...
          {blogPosts.map((post) => (
            <Link href={`/blog/${post.slug}`} key={post.slug} className="group">
              <article>
                ...
                <h3 className="text-lg font-bold text-zinc-900 leading-snug mb-2 group-hover:text-primary transition-colors line-clamp-2">
                  {post.title}
                </h3>
